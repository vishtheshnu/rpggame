var inventoryState = {
    ...
    create: function(){
        ...
        */
        ...
        //fill indeces list
        this.indeces = [];
        for(var i = 0; i < Player.Inventory.categories.length; i++)
            this.indeces.push(0);
        ...
        for(var i = 0; i < Player.Inventory.categoriesNames.length; i++){
            var text = game.add.text(0, 0, Player.Inventory.categoriesNames[i]);
            text.setTextBounds((i+1)*200, 0, 200, 100);
            text.setStyle({boundsAlignH:'center', boundsAlignV:'middle'});
            this.categories.add(text);
        }
        
        //Highlight inventory title
        this.highlight = game.add.sprite(this.cl, 25, 'highlight');
    ...
    createItems: function(){
        ...
        for(var i = 0; i < Player.Inventory[Player.Inventory.categories[this.category]].length; i++){
            var item = Player.Inventory[Player.Inventory.categories[this.category]][i];
    ...
    updateSwitch: function(){
        if(InputHandler.left.clicked){
            ...
            if(this.category < Player.Inventory.categories.length-1){
                this.category++;
                this.categories.x -= 200;
                this.createItems();
            }
